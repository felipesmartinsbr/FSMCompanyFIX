...
export function PayablesView({ onOpenNewTransaction }: PayablesViewProps) {
  const {
    transactions,
  ...
  const filteredPayables = useMemo(() => {
    return payables.filter((t) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!matchTitle && !matchSupplier && !matchCat) return false;
      }
      return true;
    });
  }, [payables, currencyFilter, statusFilter, searchQuery]);

  // Totals
  const totalPaidBRL = filteredPayables
    .filter((t) => t.status === 'pago' && t.currency === 'BRL')
    .reduce((s, t) => s + t.amount, 0);

  const totalPaidUSD = filteredPayables
    .filter((t) => t.status === 'pago' && t.currency === 'USD')
    .reduce((s, t) => s + t.amount, 0);

  const totalPendingBRL = filteredPayables
    .filter((t) => t.status === 'pendente' && t.currency === 'BRL')
    .reduce((s, t) => s + t.amount, 0);

  const totalPendingUSD = filteredPayables
    .filter((t) => t.status === 'pendente' && t.currency === 'USD')
    .reduce((s, t) => s + t.amount, 0);

  const handleExportExcel = () => {
    exportFinancialToExcel(
      filteredPayables,
      `FSM_Contas_Pagar_${new Date().toISOString().split('T')[0]}.xlsx`
    );
  };

  return (
    <div id="payables-view" className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-150">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-rose-100 text-rose-900">
              {filteredPayables.length} despesas
            </span>
          </div>
          <p className="text-xs text-neutral-500 mt-0.5">
            Custos com servidores, licenças de software, ferramentas de IA e serviços de terceiros.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-rose-700 hover:bg-rose-800 text-white rounded-lg text-xs font-bold shadow-xs transition-colors flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-neutral-200/80 shadow-2xs">
          <span className="text-[11px] text-neutral-500 font-medium block">Total Pago (BRL)</span>
          <p className="text-lg md:text-xl font-bold text-neutral-900 mt-1">
            {formatCurrency(totalPaidBRL, 'BRL')}
          </p>
          <span className="text-[10px] text-neutral-400 mt-0.5 block">Despesas quitadas em Real</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-neutral-200/80 shadow-2xs">
          <span className="text-[11px] text-neutral-500 font-medium block">A Pagar (BRL)</span>
          <p className="text-lg md:text-xl font-bold text-rose-700 mt-1">
            {formatCurrency(totalPendingBRL, 'BRL')}
          </p>
          <span className="text-[10px] text-neutral-400 mt-0.5 block">Contas pendentes em Real</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-neutral-200/80 shadow-2xs">
          <span className="text-[11px] text-neutral-500 font-medium block">Total Pago (USD)</span>
          <p className="text-lg md:text-xl font-bold text-neutral-900 mt-1">
            {formatCurrency(totalPaidUSD, 'USD')}
          </p>
          <span className="text-[10px] text-neutral-400 mt-0.5 block">SaaS / Ferramentas pagas</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-neutral-200/80 shadow-2xs">
          <span className="text-[11px] text-neutral-500 font-medium block">A Pagar (USD)</span>
          <p className="text-lg md:text-xl font-bold text-rose-700 mt-1">
            {formatCurrency(totalPendingUSD, 'USD')}
          </p>
          <span className="text-[10px] text-neutral-400 mt-0.5 block">SaaS pendentes em Dólar</span>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-neutral-50 border-b border-neutral-200 text-neutral-600 font-semibold uppercase text-[11px] tracking-wider">
              <tr>
                ...
                <th className="p-3.5">Fornecedor / Plataforma</th>
                    ...
                    <td className="p-3.5">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold capitalize ${
                          tx.status === 'pago' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                        }`}
                      >
                        {tx.status}
                      </span>
                    ...
                    <td className="p-3.5 text-right">
                      {tx.status !== 'pago' ? (
                        <button
                          ...
                          className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-800 rounded font-semibold text-xs transition-colors"
                        >
                          Marcar Pago
                        </button>
                      ) : (
